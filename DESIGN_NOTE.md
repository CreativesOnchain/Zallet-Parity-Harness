...
| `EXPECTED_MISSING` | The method returned `-32601` but is listed with `expected_missing = true` in `expected_diffs.toml`. |
...
## 6. MISSING vs EXPECTED_MISSING

By default, methods that return `-32601` (method not found) are classified as `MISSING` and trigger an exit code 1. This prevents Zallet regressions where previously implemented methods accidentally disappear.

If a method is not yet implemented by Zallet, it can be marked with `expected_missing = true` in `expected_diffs.toml`. The engine will then classify it as `EXPECTED_MISSING`, which does not trigger an exit code 1. This allows CI to stay green while acknowledging the known coverage gap.

## 7. Raw Diagnostics

When run with the `--save-raw` flag, the engine persists the unnormalized, raw JSON payloads of any method that does not `MATCH`. This allows developers to manually inspect the exact byte-for-byte responses generated by `zcashd` and `Zallet` before they hit the JSON Pointer differencing engine.

These files are stored in `reports/raw/` as `upstream_<method>.json` and `target_<method>.json`. To prevent accidental leakage of sensitive wallet data, this feature is strictly opt-in and raw logs are never written to `stdout`.
